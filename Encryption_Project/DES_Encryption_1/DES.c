...
code const uint8_t encrypt_rounds[17] = {1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1};
code const uint8_t decrypt_rounds[17] = {0 , 1 , 2 , 2 , 2 , 2 , 2 , 2 , 1 , 2 , 2 , 2 , 2 , 2 , 2 , 1};
...
*              Note     ptable is pointer to a different size array, the first of two element has the size
...
void permute(const uint8_t *ptable, const uint8_t *in, uint8_t *out) reentrant
{
    ...
    ob = ptable[1];     /* take number of bytes from the 2st element for the table , number of out bytes*/
    ...
    for(byte=0; byte<ob; byte++)
    {
        ...
        for(_bit=0; _bit<8; ++_bit)
        {
            ...
            t <<= 1;                /* select next bit  */	  // 0000 0000
            if((in[x/8]) & (0x80>>(x%8)) )		 // 0101 0001 & 0100 0000 = 0100 0000
            {
                t|=0x01;		// 0000 0000 | 0000 0001 = 0000 0001
            }
        }
        out[byte]=t;
    }
...
*
* Arguments  : key is a pointer to the key (56 bit = 7 byte) to be shifted
*
...
*
*              kr       is a pointer to array of char[6] that contain the data key.
*
